...
// resendRecord represents a single datagram with timestamps from when it was
// initially sent and most recently sent. It may be either acknowledged or
// NACKed by the other end.
...
func newRecoveryQueue() *resendMap {
	return &resendMap{unacknowledged: make(map[uint24]resendRecord)}
}
...
// acknowledge marks a datagram with the index passed as acknowledged. The
// datagram is removed from the resendMap and returned if found.
func (m *resendMap) acknowledge(index uint24) (resendRecord, bool) {
	return m.remove(index)
}

// retransmit looks up a datagram with an index from the resendMap so that it
// may be resent.
...
func (m *resendMap) clear() {
	clear(m.unacknowledged)
	m.inFlightBytes = 0
	m.retainedBytes = 0
}
